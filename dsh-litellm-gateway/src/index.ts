...
export { LiteLlmPlansReader } from './plans.ts'
export type { LiteLlmPlansFace, LiteLlmPlanStatus, LiteLlmPlansSnapshot } from './plans.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  const profiles = (): ReadonlyMap<string, import('@deepseek-ai/dsh-llm-pi-ai').ResolvedPiAiProviderProfile> => {
    ...
    return resolveProfiles({ [resolved.provider]: modelProfiles(resolved, modelInfo.uniqueModelNames) })
  }
  ...
  const adapter = new PiAiAdapter({
    ...
    onResponse: (request, response) => usage.observeResponse(request, response),
  })
  ...
  const ensure = (): void => {
    ...
    const entries: LlmConfigurableProvider[] = [{
      ...
      settingsNs: NS,
      settingsPath: [],
      catalog: providerCatalog(resolved, modelInfo.uniqueModelNames),
    }]
