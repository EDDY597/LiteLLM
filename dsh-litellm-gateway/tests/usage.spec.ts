...
describe('LiteLLM usage ledger', () => {
  it('aggregates virtual-model requests and disjoint token buckets', () => {
    ...
    expect(snapshot.totals).toEqual({
      requests: 2, successes: 1, failures: 1, inputTokens: 12, outputTokens: 7,
      cacheReadTokens: 3, cacheWriteTokens: 1,
    })
    expect(snapshot.rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ provider: 'litellm-gateway', model: 'dsh-balanced', requests: 1, successes: 1 }),
      expect.objectContaining({ provider: 'litellm-gateway', model: 'dsh-cost', requests: 1, failures: 1 }),
    ]))
  })
  ...
  it('uses the exact response-header deployment and ignores an echoed route alias', () => {
    const ledger = new LiteLlmUsageLedger()
    ledger.record(request, [routedFinish('dsh-balanced')])
    expect(ledger.currentActive()).toBeNull()

    ledger.stageResponse(request, 'opencode/deepseek-v4-flash')
    ledger.record(request, [routedFinish('dsh-balanced')])
    expect(ledger.currentActive()).toMatchObject({
      model: 'dsh-balanced', responseModel: 'opencode/deepseek-v4-flash',
    })
  })
