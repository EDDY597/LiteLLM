...
Model selection plugin, browser half: TWO entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`). For ordinary sessions, the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance. The compact composer trigger opens a two-level menu whose root lists the directory's buckets — Routes and Route models come from any provider-declared routing slice riding `SessionModels.routing`, DSH models from every other provider group — and each bucket drills into its own provider-grouped list; the selected exact model supplies adapter-owned effort names, descriptions, and default, rendered as an indented subgroup directly beneath whichever model row is currently selected. `/model` applies the selected model's default effort, and the composer can then choose any advertised effort; deployments without a routing slice see only the DSH models pane and behave like a plain grouped list.

The Host-reported provider/model/reasoning `ModelSelection` is the single selection fact, but it is echoed only when the exact provider/model pair remains in the advertised groups; an absent catalog row leaves the routable selection intact while the trigger prompts `Select model`, no stale row is synthesized, and no effort subgroup appears until the user picks an advertised model. Directory loads and selections share a generation counter so an older response never overwrites a newer one; a connection reset drops every resident projection and repulls the Host-restored selection before display. Provider-local metadata failures list inline while usable groups stay selectable, and selection failures retain the prior selection and directory.
...
- **No arbitrary effort input** — the composer offers only the exact model's adapter-advertised levels; an adapter without reasoning metadata leaves the effort subgroup absent.
- **Routing slices are display metadata only** — when a routing provider's catalog lookup fails its slice disappears from the Routes/Route models panes entirely (the failure row explains why), and credential-unresolved entries stay visible but inert; selection never invents a fallback route.
