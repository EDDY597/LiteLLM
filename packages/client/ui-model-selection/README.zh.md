...
模型选择插件（浏览器侧）：**两个入口共用一份会话级目录**，由 `ModelDirectoryResolver`（`ctx.modelDirectories`）持有。对于普通会话，`/model` popupSelect 贡献项（经 `ctx.commandUi` 注册）与 composer 的具名 `conversation.input.model` slot 都通过同一个 `ModelDirectory` 实例，经 `session.models` 加载会话的建议目录，并经 `session.selectModel` 提交。紧凑型 composer 触发器会打开两级菜单，根级列出目录的分组——「路由」与「路由模型」来自提供方经由 `SessionModels.routing` 发布的路由切片，「DSH模型」来自其余全部提供方分组——每组各自下钻到按提供方分组的列表；所选具体模型提供由其适配器持有的推理强度名称、说明和默认值，并以缩进子组的形式直接渲染在当前选中模型行之下。`/model` 应用所选模型的默认推理强度，composer 随后可以选择任一已公布的推理强度；未发布路由切片的部署只会看到 DSH 模型一组，行为等同普通分组列表。

Host 报告的 `ModelSelection` 是唯一的选择事实，其中包含提供方、模型与推理（reasoning）强度；但只有当该提供方／模型对仍在已公布分组中时才会回显。目录行缺席时，可路由的选择保持不变，但触发器会提示 `Select model`；系统不会合成陈旧行，且在用户选择已公布的模型之前不会显示推理强度子组。目录加载与选择共享一个代次计数器，旧响应不会覆盖新结果；连接重置会丢弃所有常驻目录投影，并在显示前重新拉取 Host 恢复的选择。各提供方的元数据获取失败会内联列出，同时可用分组仍可选择；选择失败会保留先前的选择和目录。
...
- **不能任意输入推理强度**——composer 仅提供确切模型由适配器公布的推理强度；适配器没有推理元数据时不显示推理强度子组。
- **路由切片仅是展示元数据**——路由提供方的目录查询失败时，其切片整体从「路由」「路由模型」两组消失（失败行会说明原因），凭据未解析的条目保持可见但不可选；选择逻辑绝不虚构回退路由。
