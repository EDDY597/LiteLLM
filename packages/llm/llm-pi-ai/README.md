...
Plugins that embed this adapter can construct `PiAiAdapter` directly and use the exported `resolveProfiles()`, `credentialStoreFrom()`, and `authContextFrom()` helpers. The embedding plugin owns its profile policy and route registration; these helpers retain the same profile validation and pi-ai authentication behavior without mounting a second `llm-pi-ai` plugin instance.
