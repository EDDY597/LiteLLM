...
嵌入该适配器的插件可以直接构造 `PiAiAdapter`，并使用导出的 `resolveProfiles()`、`credentialStoreFrom()` 与 `authContextFrom()` helper。嵌入插件自行管理 profile 策略与路由注册；这些 helper 无需挂载第二个 `llm-pi-ai` 插件实例，即可保留相同的 profile 校验与 pi-ai 认证行为。
